...
<head>
    ...
    <title>@yield('title', 'Espace Vendeur') | Luxe Maquette</title>
...
<body class="bg-background text-on-surface min-h-screen flex">
    ...
    <aside class="fixed left-0 top-0 h-full w-64 bg-primary flex flex-col z-50">
        ...
        $app = $application
        ?? (auth()->check()
        ? \App\Models\ArtisanApplication::where('user_id', auth()->id())->first()
        : null);
        @endphp
        ...
        <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
            ...
            <a href="{{ route('artisan.products.index') }}" class="nav-link {{ request()->routeIs('artisan.products.*') ? 'active' : '' }}">
                ...
                Mes Produits
                @php $prodCount = $app ? $app->products()->count() : 0; @endphp
                @if($prodCount > 0)
            ...
            <a href="{{ route('artisan.orders.index') }}" class="nav-link {{ request()->routeIs('artisan.orders.*') ? 'active' : '' }}">
                ...
                Commandes
                @php $pendingOrders = $app?->orderItems()?->where('item_status', 'pending')?->count() ?? 0; @endphp
                @if($pendingOrders > 0)
            ...
            <a href="{{ route('chat.index') }}" class="nav-link {{ request()->routeIs('chat.*') ? 'active' : '' }}">
                ...
                ->get()
                ->sum(fn($c) => $c->unreadCountFor(auth()->id()));
                }
                @endphp
                @if($unread > 0)
