...
<script>
    ...
    const categoriesData = {
        !!$categories - > keyBy('id') - > map(fn($c) => $c - > subcategories - > map(fn($s) => ['id' => $s - > id, 'name' => $s - > name])) - > toJson() !!
    };
    ...
    //script pour afficher/cacher le champ selon le badge sélectionné
    document.getElementById('condition_label').addEventListener('change', function() {
        const originalPriceField = document.getElementById('original-price-field');
        const priceHint = document.getElementById('price-hint');

        if (this.value === 'promotion') {
            originalPriceField.classList.remove('hidden');
            priceHint.textContent = 'Entrez le prix PROMOTIONNEL en FCFA';
            priceHint.classList.add('text-error');
        } else {
            originalPriceField.classList.add('hidden');
            priceHint.textContent = 'Entrez le prix en FCFA';
            priceHint.classList.remove('text-error');
        }
    });
</script>
@endsection
