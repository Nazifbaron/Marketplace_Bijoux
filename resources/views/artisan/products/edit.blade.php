    ...
    <section class="bg-surface border border-outline-variant/30 p-7 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="price" value="{{ old('price', $product->price) }}" step="any" min="100"
                    class="w-full border-0 border-b border-outline-variant py-3 px-0 focus:ring-0 focus:border-secondary transition-all text-lg" />
            </div>
            <div id="original-price-field" class="{{ old('condition_label', $product->condition_label ?? 'none') === 'promotion' ? '' : 'hidden' }}">
                ...
                <input type="number" name="original_price" value="{{ old('original_price', $product->original_price ?? '') }}"
                    ...
                    class="w-full border-b border-outline-variant py-3 px-0 bg-transparent focus:ring-0 focus:border-secondary transition-all" />
                <p class="text-[10px] text-on-surface-variant/60 mt-1">
                    L'ancien prix affiché barré — la réduction sera calculée automatiquement
                </p>
            </div>
            <div>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                @foreach(['none' => 'Aucun', 'promotion' => 'Promotion', 'limited_edition' => 'Édition Limitée', 'made_to_order' => 'Sur Commande'] as $value => $label)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ── 5. PRIX ORIGINAL — afficher si badge = promotion ──
    function toggleOriginalPrice() {
        const selected = document.querySelector('input[name="condition_label"]:checked');
        const field    = document.getElementById('original-price-field');
        if (!field) return;

        if (selected && selected.value === 'promotion') {
    ...
    document.querySelectorAll('input[name="condition_label"]').forEach(function(radio) {
        radio.addEventListener('change', toggleOriginalPrice);
    });
...
<script>
// ── SUPPRESSION IMAGE EXISTANTE (doit rester globale) ──
